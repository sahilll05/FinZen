...
def _fetch_news_risk_adjustment(country_code: str) -> dict:
    """
    In production, fetch real GDELT data and compute adjustment.
    For now, returns small random adjustments.
    """
    import random
    return {
        "war_risk": random.uniform(-0.5, 0.5),
        "sanctions_risk": random.uniform(-0.3, 0.3),
        "regulatory_risk": random.uniform(-0.3, 0.3),
        "economic_risk": random.uniform(-0.5, 0.5),
        "political_risk": random.uniform(-0.3, 0.3),
        "currency_risk": random.uniform(-0.5, 0.5),
    }
...
def get_globe_data() -> list:
    """Return risk data for all globe countries."""
    result = []
    for entry in GLOBE_COUNTRIES:
        code = entry["code"]
        try:
            risk = analyze_country_risk(code)
            result.append({
                "code": code,
                "lat": entry["lat"],
                "lng": entry["lng"],
...
COUNTRY_STOCK_MAP: Dict[str, list] = {
    ...
    "JP":  [{"name": "Nikkei",    "ticker": "^N225"}, {"name": "TOPIX",    "ticker": "^TOPX"}],
...
def get_country_stocks(country_code: str) -> dict:
    """Fetch real-time stock index data for a country."""
    ...
    for entry in tickers:
        ...
        try:
            ...
            stocks.append({"name": name, "ticker": ticker, "price": None, "change_pct": 0.0})
