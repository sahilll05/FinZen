                  ...
                  >
                    <td className="px-5 py-4">
                      <div className="font-mono font-bold text-text-primary text-sm flex gap-2">
                        {h.ticker}
                        <span className="text-[9px] font-sans bg-border-light/40 px-1.5 rounded-sm border border-border-base text-text-dim flex items-center">{h.sector || "Unknown"}</span>
                      </div>
                      <div className="text-[11px] text-text-secondary font-sans truncate max-w-[120px] mt-0.5">{h.company_name || 'Stock Asset'}</div>
                    </td>
                    <td className="px-5 py-4">
                      <div className="font-sans font-bold text-sm text-text-primary">${(h.current_price || h.avg_cost).toFixed(2)}</div>
                      <div className={`text-[11px] font-bold ${isProfitable ? 'text-accent-sage' : 'text-accent-rose'}`}>
                        {isProfitable ? '+' : '-'}${Math.abs(h.day_change || 1.2).toFixed(2)} ({isProfitable ? '+' : ''}{(h.day_change_pct || 1.5).toFixed(2)}%)
                      </div>
                    </td>
      ...
      {activeHolding && (
        <SoftCard className="w-full lg:w-1/3 flex flex-col p-0 shadow-2xl animate-in slide-in-from-right-8 bg-surface border-l-4 border-l-accent-indigo relative overflow-hidden">
          ...
          <div className="p-6 flex-1 overflow-y-auto space-y-6 bg-surface">
            ...
            <Button onClick={() => onRemoveHolding(activeHolding.$id)} variant="outline" className="w-full text-accent-rose border-accent-rose hover:bg-accent-rose hover:text-white transition-all shadow-sm">
               Liquidate / Remove Holding
            </Button>
          </div>
        </SoftCard>
      )}
