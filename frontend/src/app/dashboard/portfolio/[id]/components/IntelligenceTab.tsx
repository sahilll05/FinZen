...
function DiversificationBar({ score }: { score: number }) {
  ...
  const getColor = (s: number) => {
    if (s <= 30) return 'bg-accent-rose';
    if (s <= 70) return 'bg-accent-amber';
  ...
  return (
    <div className="group relative mt-6 pt-5 border-t border-border-light">
      <div className="flex justify-between items-center mb-2">
        <span className="text-xs font-bold uppercase tracking-widest text-text-secondary flex items-center gap-1.5 cursor-help">
          Diversification 
          <span className="w-3 h-3 rounded-full border border-border-strong flex items-center justify-center text-[8px]">?</span>
        </span>
        <div className="text-right">
          <span className="font-mono text-sm text-text-primary font-bold">{score} / 100</span>
          <span className="text-[10px] text-text-dim ml-2 uppercase font-bold">({getLabel(score)})</span>
        </div>
      </div>
      
      <div className="h-2 w-full bg-elevated rounded-full overflow-hidden border border-border-light shadow-inner">
        <div 
          className={`h-full transition-all duration-1000 ease-out rounded-full ${getColor(score)}`}
          style={{ width: `${width}%` }} 
        />
      </div>
      
      {/* Tooltip */}
      <div className="absolute opacity-0 group-hover:opacity-100 transition-opacity bottom-full mb-2 left-0 right-0 z-10 pointer-events-none">
        <div className="bg-surface border border-border-strong px-3 py-2 rounded-lg text-[10px] leading-tight text-text-secondary shadow-xl w-max max-w-[250px]">
          Diversification shows how spread out your investments are across sectors. A high score means lower single-point failure risk.
        </div>
      </div>
    </div>
  );
...
export function IntelligenceTab({ portfolio, holdings }: { portfolio: any, holdings: any[] }) {
  ...
  // Calculate Diversification Score mathematically from Concentration
  const diversificationScore = useMemo(() => {
    if (holdings.length === 0) return 0;
    if (holdings.length === 1) return 15;
    const topClaim = report?.claims.find(c => c.id === 'sector_concentration');
    const topCount = topClaim?.evidence?.value?.top_sector_count || 1;
    const pct = (topCount / holdings.length) * 100;
    const rawScore = 100 - pct;
    return Math.max(0, Math.min(100, Math.round(rawScore + (holdings.length * 3)))); // bonus for # holdings
  }, [report, holdings]);
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-12">
        <SoftCard className="p-6 bg-surface border border-accent-rose/30">
          ...
          {/* Animated Diversification Score Component */}
          {reliabilityGatePassed && holdings.length > 0 && (
            <DiversificationBar score={diversificationScore} />
          )}
        </SoftCard>
